...
import type { TableColumns, Operations } from 'u-element-plus'
import type { TableDataItem } from '../table/useTableData'

const { tableData, statusOptions, getOptionText, getOptionType } = useTableData(6, true)

// 操作按钮列表
const operations = ref<Operations<TableDataItem>>([
  // 根据状态判断是否显示编辑按钮
  {
    label: '编辑',
    name: 'edit',
    buttonAttrs: (row, index) => ({
      onClick: () => {
        ElMessage.warning(`第${index + 1}行的【编辑】按钮点击事件触发～`)
      },
    }),
  },
  {
    ...
    buttonAttrs: (row, index) => ({
      type: 'danger',
      onClick: () => {
        ElMessage.warning(`第${index + 1}行的【删除】按钮点击事件触发～`)
      },
    }),
    confirmAttrs: (row, index) => ({
      ...
      width: 170,
      onConfirm: () => {
        ElMessage.warning(`第${index + 1}行的【删除】按钮确认事件触发～`)
      },
      onCancel: () => {
        ElMessage.warning(`第${index + 1}行的【删除】按钮取消事件触发～`)
      },
    }),
  },
  {
    ...
    buttonAttrs: (row, index) => ({
      type: 'primary',
      onClick: () => {
        ElMessage.warning(`第${index + 1}行的【详情】按钮点击事件触发～`)
      },
    }),
  },
])
